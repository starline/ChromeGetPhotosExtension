...
- **GetProducts** — parses Taobao / Tmall shop pages and extracts product cards: image, title, sales count, price, and product link.
...
5. Use the **link** icon to copy a URL, the **copy** icon to copy the image itself, the **open** icon to open the link in a new tab, or the **delete forever** icon to remove the image from the current list. Hover tooltips (Bootstrap) show action labels on buttons and other titled controls.
...
- `src/sidepanel.js` — side panel UI (tool switcher, settings, lists, filters, sort by image size / price / sales, copy link/image, open, remove from list, Bootstrap tooltips).
