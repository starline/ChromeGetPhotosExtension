...
3. Use the **square icons** at the top to switch tools:
   ...
   - **GetProducts** — on a Taobao/Tmall shop page click **Получить товары** to list products (image, title, price, sales, link). Use **По цене** / **По продажам** above the list to sort (click again to reverse direction). Open a product in the current tab, then use **Фотографии** on the highlighted row to fill a thumbnail grid from that product page. Product photos keep only images at least as wide as the default min width in Settings.
